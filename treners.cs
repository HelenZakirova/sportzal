...
namespace sportzal
{
    public partial class treners : Form
    {
        ...
        private void button3_Click(object sender, EventArgs e)
        {
            ...
            {
                MySqlConnection conn = new MySqlConnection("Server=localhost;Database=sportzal;User=root;Password=2375425;");
                MySqlCommand cmd = new MySqlCommand("update treners set id_trener ='" + textBox1.Text + "', FIO = '" + textBox2.Text + "', oklad = '" + textBox3.Text + dataGridView1[0, dataGridView1.CurrentRow.Index].Value.ToString() + ";", conn);
                conn.Open();
                cmd.ExecuteReader();
                conn.Close();
                getinfo("select * from treners;");
            }
            catch (Exception ex)
            {
